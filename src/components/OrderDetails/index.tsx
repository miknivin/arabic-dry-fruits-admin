...
export default function OrderDetails({ orderId }: OrderDetailsProps) {
  ...
  const order = orderDetails as any;

  // Determine which shipping object actually has data
  ...
  const resolvedShipping = hasShippingInfo
    ? shippingInfo
    : hasShippingAddress
    ? shippingAddress
    : null;
  ...
  return (
    <div className="px-4 py-14 2xl:container md:px-6 2xl:mx-auto 2xl:px-20">
      ...
      <OrderHeader
        ...
        orderStatus={orderDetails.orderStatus || order.status || 'Processing'}
      ...
      <div className="mt-10 flex w-full flex-col items-stretch justify-center space-y-4 md:space-y-6 xl:flex-row xl:space-x-8 xl:space-y-0">
        <div className="flex w-full flex-col items-start justify-start space-y-4 md:space-y-6 xl:space-y-8">
          <div className="flex w-full flex-col items-start justify-start bg-gray-50 px-4 py-4 dark:bg-gray-800 md:p-6 md:py-6 xl:p-8">
            ...
            {cartItems.length === 0 && (
              <p className="mt-4 text-sm text-gray-500">No items found in this order.</p>
            )}
        ...
        <CustomerDetails customer={resolvedShipping} />
      </div>
    </div>
  );
}
