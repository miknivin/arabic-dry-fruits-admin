...
const ProductItem: React.FC<ProductItemProps> = ({ product, waybill }) => {
  const { name, quantity, price, variant } = product;
  const image = product.image || (product as any).imageUrl;
    ...
    <div className="mt-4 flex w-full flex-col items-start justify-start md:mt-6 md:flex-row md:items-center md:space-x-6 xl:space-x-8">
      ...
      <div className="flex w-full flex-col items-start justify-between space-y-4 border-b border-gray-200 pb-8 dark:border-gray-700 md:flex-row md:space-y-0">
        <div className="flex w-full flex-col items-start justify-start space-y-3">
          ...
          {(typeof variant === "object" && variant?.size) || (typeof variant === "string" && variant) ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800 ring-1 ring-emerald-400/30">
              📦 {typeof variant === "object" ? variant.size : variant}
            </span>
          ) : null}
          {customNameToPrint && (
