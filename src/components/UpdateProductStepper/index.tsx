...
import { isProductValid } from "@/utils/validation/productValidators/entireProduct";
import {
...
import { validateProduct } from "@/utils/validation/details";
  ...
  const handleSubmit = async () => {
    ...
    const formattedProduct = {
      ...
      images: product.images
        .map((img) =>
          typeof img === "object" && (img as any).url ? { url: (img as any).url, alt: (img as any).alt || "" } : { url: String(img || ""), alt: "" }
        )
        .filter((img) => img.url),
    };
