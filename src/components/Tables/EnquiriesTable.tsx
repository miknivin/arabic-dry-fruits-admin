...
const EnquiriesTable = () => {
  ...
  const filteredEnquiries = useMemo(() => {
    ...
    return enquiries.filter((enquiry: Enquiry) => {
      const name = (enquiry.name || enquiry.fullName || "").toLowerCase();
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
              ...
              <tr key={key}>
                <td className="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                  <h5 className="font-medium text-black dark:text-white">
                    {enquiry.name || enquiry.fullName || <span className="text-gray-400 italic">N/A</span>}
                  </h5>
                </td>
        ...
        <ReusableModal
          ...
          formContent={
            <div className="p-4 space-y-3">
              ...
              {currentEnquiry.subject && (
                <div>
                  <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Subject</p>
                  <p className="text-sm text-gray-700 dark:text-gray-300">{currentEnquiry.subject}</p>
                </div>
              )}
              <div>
      ...
      {isDeleteModalOpen && currentEnquiry && (
        <ReusableAlert
          ...
          content={`Are you sure you want to delete the enquiry from "${currentEnquiry.name || currentEnquiry.fullName || "N/A"}"?`}
          func={handleDelete}
